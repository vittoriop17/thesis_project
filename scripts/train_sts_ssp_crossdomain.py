"""
see original code at https://github.com/UKPLab/sentence-transformers/blob/master/examples/training/data_augmentation/train_sts_qqp_crossdomain.py
The script shows how to train Augmented SBERT (Domain-Transfer/Cross-Domain) strategy for STSb-SSP dataset.
For our example below we consider STSb (source) and SSP (target) datasets respectively.
Where SSp stands for Sentence-sentence pairs. It's the dataset extracted from the training data. The sentence pairs
are randomly generated from the training sentences_wo_punct.
Methodology:
...
2. Cross-Encoder is used to label SSP training (target) dataset (Assume no labels/no annotations are provided).
3. Bi-encoder aka SBERT is trained over the labeled SSP (target) dataset.
...
bi_encoder_path = 'output/bi-encoder/ssp_cross_domain_'+model_name.replace("/", "-")+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
##################################################################
#
# Step 2: Label SSp train dataset using cross-encoder (BERT) model
#
##################################################################
